...
$SharedCppLib = "$BackendArtifactDir/$Architecture/libc++_shared.so"
$AudioEffectsConfig = "./audio_effects.xml"
...
ExecuteAdbCommand -logMsg "Pushing Shared C++ Lib to Device..." -failMsg "Failed to Push Shared C++ Lib to Device!" -command "push $SharedCppLib $VendorLib"
...
ExecuteAdbCommand -logMsg "Setting Menrva Lib SEContext..." -failMsg "Failed to set Menrva Lib SEContext!" -command "shell chcon -v u:object_r:vendor_file:s0 $SoundFxLib/$MenrvaLib"
ExecuteAdbCommand -logMsg "Setting FFTW Lib SEContext..." -failMsg "Failed to set FFTW Lib SEContext!" -command "shell chcon -v u:object_r:vendor_file:s0 $VendorLib/$Fftw3Lib"
ExecuteAdbCommand -logMsg "Setting KissFFT Lib SEContext..." -failMsg "Failed to set KissFFT Lib SEContext!" -command "shell chcon -v u:object_r:vendor_file:s0 $VendorLib/$KissFftLib"
ExecuteAdbCommand -logMsg "Setting Shared C++ Lib SEContext..." -failMsg "Failed to set Shared C++ Lib SEContext!" -command "shell chcon -v u:object_r:vendor_file:s0 $VendorLib/$SharedCppLib"
ExecuteAdbCommand -logMsg "Setting Audio Effects Config File SEContext..." -failMsg "Failed to set Audio Effects Config File SEContext!" -command "shell chcon -v u:object_r:vendor_configs_file:s0 $VendorEtc/$AudioEffectsConfig"
